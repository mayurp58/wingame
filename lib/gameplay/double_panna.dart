...
class DoublePanna extends StatefulWidget {
  ...
  final int id;
  final String? date;
  const DoublePanna({Key? key, this.title, this.session, required this.id,required this.date}) : super(key: key);

  @override
  State<DoublePanna> createState() => _DoublePannaState();
}

class _DoublePannaState extends State<DoublePanna> with SingleTickerProviderStateMixin, AutomaticKeepAliveClientMixin<DoublePanna>{
                          ...
                          onPressed: (){
                            ...
                            if (validateAndSave() && itemDetail.length > 0)
                            {
                              Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                      builder: (context) => SubmitBid(title: widget.title,session:widget.session,id: widget.id,date: widget.date,itemDetail: itemDetail,image: "double_panna.png",game_name: "Double Panna",total: total.toString(),))
