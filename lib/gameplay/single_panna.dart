...
class SinglePanna extends StatefulWidget {
  ...
  final int id;
  final String? date;
  const SinglePanna({Key? key, this.title, this.session, required this.id,required this.date}) : super(key: key);

  @override
  State<SinglePanna> createState() => _SinglePannaState();
}

class _SinglePannaState extends State<SinglePanna> with SingleTickerProviderStateMixin, AutomaticKeepAliveClientMixin<SinglePanna>{
  ...
  Widget build(BuildContext context) {
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
          ...
          child : Column(
            children: [
              GameplayHeader(id:widget.id.toString(),title: widget.title,type: "Single Pana",session: widget.session,date: widget.date,),
              //////////////////////////////Heade Ends Here////////////////////////////
              Container(
                color: Colors.black,
                          ...
                          onPressed: (){
                            ...
                            if (validateAndSave() && itemDetail.length > 0)
                            {
                              Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                      builder: (context) => SubmitBid(title: widget.title,session:widget.session,id: widget.id,date: widget.date,itemDetail: itemDetail,image: "single_panna.png",game_name: "Single Panna",total: total.toString(),))
