...
class TriplePanna extends StatefulWidget {
  ...
  final int id;
  final String? date;
  const TriplePanna({Key? key, this.title, this.session, required this.id,required this.date}) : super(key: key);

  @override
  State<TriplePanna> createState() => _TriplePannaState();
}

class _TriplePannaState extends State<TriplePanna> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            GameplayHeader(id:widget.id.toString(),title: widget.title,type: "Triple Pana",session: widget.session,date: widget.date,),
            //////////////////////////////Heade Ends Here////////////////////////////
            Container(
              //height: ,
                                      ...
                                      onPressed: (){
                                        if (validateAndSave() && itemDetail.length > 0)
                                        {
                                          Navigator.push(
                                              context,
                                              MaterialPageRoute(
                                                  builder: (context) => SubmitBid(title: widget.title,session:widget.session,id: widget.id,date: widget.date,itemDetail: itemDetail,image: "tripple_panna.png",game_name: "Triple Panna",total: total.toString(),))
