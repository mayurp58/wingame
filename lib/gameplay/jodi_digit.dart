...
class JodiDigit extends StatefulWidget {
  ...
  final int id;
  final String? date;
  const JodiDigit({Key? key, this.title, this.session, required this.id,required this.date}) : super(key: key);

  @override
  State<JodiDigit> createState() => _JodiDigitState();
}

class _JodiDigitState extends State<JodiDigit> {
    ...
    return Scaffold(
      appBar: PreferredSize(
          ...
          body:SingleChildScrollView(
            child: Column(
              children: [
                GameplayHeader(id:widget.id.toString(),title: widget.title,type: "Jodi Digit",session: "",date: widget.date,),
                //////////////////////////////Heade Ends Here////////////////////////////
                Container(
                  //height: ,
                          ...
                          onPressed: (){
                              ...
                              if (validateAndSave() && itemDetail.length > 0)
                              {
                                Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                        builder: (context) => SubmitBid(title: widget.title,session:widget.session,id: widget.id,date: widget.date,itemDetail: itemDetail,image: "jodi_digit.png",game_name: "Jodi Digit",total: total.toString(),))
