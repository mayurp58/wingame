...
class Sup_red_bracket extends StatefulWidget {
  ...
  final int id;
  final String? date;
  const Sup_red_bracket({Key? key, this.title, this.session, required this.id, this.date}) : super(key: key);

  @override
  State<Sup_red_bracket> createState() => _Sup_red_bracketState();
}

class _Sup_red_bracketState extends State<Sup_red_bracket> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body:SingleChildScrollView(
        child: Column(
          children: [
            Sup_gameplay_header(id:widget.id.toString(),title: widget.title,type: "Red Bracket",session: "",date: widget.date,),
            //////////////////////////////Heade Ends Here////////////////////////////
            Container(
              //height: ,
                          ...
                          onPressed: (){
                            ...
                            if (validateAndSave() && itemDetail.length > 0)
                            {
                              Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                      builder: (context) => Sup_submit_bid(title: widget.title,session:widget.session,id: widget.id,date: widget.date,itemDetail: itemDetail,image: "red_bracket.png",game_name: "Red Bracket",total: total.toString(),))
